...
const ReviewCard = ({ review }: { review: any }) => {
  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-7 shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-gray-100/80 min-w-[260px] max-w-[260px] sm:min-w-[360px] sm:max-w-[360px] mx-1.5 sm:mx-3 flex flex-col gap-3 sm:gap-5 transition-all duration-300 hover:shadow-[0_8px_32px_rgba(91,200,63,0.12)] hover:-translate-y-1 hover:border-brand-green/30 relative overflow-hidden group h-full">
      
      {/* Quotation watermark */}
      <div className="absolute top-3 right-3 sm:top-5 sm:right-6 text-gray-100 opacity-60 transition-all duration-300 group-hover:opacity-100 group-hover:text-brand-green/5 group-hover:scale-110 pointer-events-none">
        <svg className="w-6 h-6 sm:w-10 sm:h-10" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="flex items-start justify-between relative z-10">
        <div className="flex items-center gap-2.5 sm:gap-3.5">
          <div className={`flex-shrink-0 w-8 h-8 sm:w-11 sm:h-11 rounded-full flex items-center justify-center font-bold text-sm sm:text-lg ${avatarClass}`}>
            {review.reviewer_name.charAt(0).toUpperCase()}
          </div>
          <div>
            <div className="flex items-center gap-1.5">
              <h4 className="font-semibold text-gray-900 text-[13px] sm:text-[15px]">{review.reviewer_name}</h4>
            ...
            <div className="flex items-center gap-0.5 mt-1">
              {[...Array(review.star_rating)].map((_, i) => (
                <Star key={i} className="w-[10px] h-[10px] sm:w-3 sm:h-3 fill-[#FBBC05] text-[#FBBC05]" />
              ))}
            </div>
          </div>
        </div>
      ...
      <p className="text-gray-600 text-[12.5px] sm:text-[14.5px] leading-relaxed whitespace-normal break-words overflow-hidden text-ellipsis line-clamp-3 sm:line-clamp-4 relative z-10 flex-1">
        {review.review_text}
      </p>
    </div>
  );
  ...
  return (
    <section className="py-12 md:py-24 overflow-hidden relative bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8 sm:mb-12 relative z-10 text-center">
        ...
        <h2 className="text-2xl md:text-5xl font-extrabold text-heading leading-tight font-display mb-3 sm:mb-4">
          Trusted by thousands of <br />
          <span className="text-brand-green">happy customers.</span>
        </h2>
        <p className="text-body max-w-2xl mx-auto text-[13px] sm:text-[15px]">
          Our commitment to transparent, fast, and reliable financial services is reflected in our leading regional Google rating.
        </p>
      </div>

      <div className="relative w-full overflow-hidden flex flex-col gap-3 sm:gap-6 py-2 sm:py-4">
        {/* Gradients to hide edges */}
        <div className="absolute top-0 left-0 h-full w-12 md:w-56 bg-gradient-to-r from-white to-transparent pointer-events-none z-10" />
        <div className="absolute top-0 right-0 h-full w-12 md:w-56 bg-gradient-to-l from-white to-transparent pointer-events-none z-10" />
